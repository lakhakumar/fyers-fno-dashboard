 ...
 */
const FYERS_SAFE_REQUESTS_PER_MINUTE = 120;
const FYERS_MIN_REQUEST_GAP_MS = 400;
...
function noteFyers429() {
  fyers429Count += 1;
  const cool = Math.min(60000, 5000 * Math.pow(2, Math.min(fyers429Count - 1, 3)));
  fyersCooldownUntil = Math.max(fyersCooldownUntil, Date.now() + cool);
  log(
    `FYERS rate-limit cooldown ${Math.round(cool / 1000)}s (429 count=${fyers429Count})`,
    'warn'
  );
}
...
function missingRankTimes(date) {
  ...
  return expected.filter(t => {
    ...
    return n < 10; // only truly missing / broken buckets
  });
}

/**
 * Slow, resumable gap-fill. Waits out FYERS cooldown, avoids bulk quote burst,
 * processes one stock at a time, pauses hard on 429 and retries later.
 */
  ...
  historyRebuildPromise = (async () => {
    try {
      if (Date.now() < fyersCooldownUntil) {
        await sleep(Math.max(0, fyersCooldownUntil - Date.now()) + 1000);
      }
      if (!state.fyers.appId) return;

      await loadDbHistory(iso);
      const gaps = missingRankTimes(iso);
      if (!gaps.length) {
        log(`Timeline complete for ${iso} — nothing to gap-fill.`);
        return;
      }

      const universe = await ensureUniverse();
      const missingSet = new Set(gaps);
      log(
        `Gap-fill starting for ${iso}: ${gaps.length} times, ${universe.length} stocks.`
      );
      ...
      for (const stock of universe) {
        ...
        if (Date.now() < fyersCooldownUntil) {
          const w = fyersCooldownUntil - Date.now() + 30000;
          log(`Gap-fill paused ${Math.round(w / 1000)}s (rate limit).`, 'warn');
          await sleep(w);
        }
        try {
          const cs = await history5m(stock, appId, token, iso);
          // Prefer previous-day close: open of first bar is weak but avoids extra quote burst
          let prev = null;
          if (cs.length >= 1) {
            // Use first bar's open as session reference only if we have no better prev
            prev = Number(cs[0].open);
          }
          // Better: if 09:20 snapshot exists for this key, back out prev from pct+close
          const g920 = state.history.get('09:20')?.gainers || [];
          const row920 = g920.find(x => x.key === stock.key);
          if (row920 && Number.isFinite(row920.close) && Number.isFinite(row920.pct) && row920.pct !== -100) {
            const p = row920.pct / 100;
            if (p > -0.999) prev = row920.close / (1 + p);
          }
          ...
          if (/429|rate limit/i.test(e.message)) {
            hardFail = true;
            log(
              `Gap-fill hit 429 after ${done}/${universe.length} stocks — will resume later.`,
              'warn'
            );
            break;
          }
        ...
        if (done % 25 === 0) {
          log(`Gap-fill progress: ${done}/${universe.length} stocks`);
        }
      }
      ...
      if (filled.length) {
        log(
          `Gap-fill saved ${filled.length} times for ${iso}. Timeline: ${[...state.history.keys()].sort().join(', ')}`
        );
        broadcastDashboard();
      }

      // If stopped early due to 429, schedule one automatic resume
      if (hardFail && state.fyers.appId) {
        const retryIn = Math.max(180000, fyersCooldownUntil - Date.now() + 60000);
        log(`Scheduling gap-fill resume in ${Math.round(retryIn / 1000)}s…`);
        setTimeout(() => {
          if (!state.fyers.appId) return;
          fillMissingSnapshotsFromFyers(
            state.fyers.appId,
            state.fyers.token,
            iso
          ).catch(e => log(`Gap-fill resume: ${e.message}`, 'warn'));
        }, retryIn);
      } else if (!hardFail) {
...
async function route(req, res) {
  ...
  try {
    ...
    if (pathName === '/api/login' && req.method === 'POST') {
      ...
      // Validate with one NIFTY quotes call (blocking login — same as original flow)
      try {
        await fyersGet(
          DATA_HOST,
          '/quotes',
      ...
      // Scanner metrics in background (does not block login response further)
      setImmediate(() => {
        (async () => {
          try {
    ...
    if (pathName === '/api/dashboard' && req.method === 'GET') {
      ...
      if (!state.fyers.appId && appId && token) {
        ...
        log('Session restore from browser headers.');
        try {
          await ensureUniverse();
          if (!isTradingSessionDay() || !regularSessionStarted()) {
