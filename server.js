...
async function route(req, res) {
  ...
  try {
    ...
    if (pathName === '/api/login' && req.method === 'POST') {
      ...
      // Single validation call — no stacked retries (those caused 429 storms)
      try {
        if (Date.now() < fyersCooldownUntil) {
          const sec = Math.ceil((fyersCooldownUntil - Date.now()) / 1000);
          return send(res, 429, {
            ok: false,
            error:
              `FYERS rate limit is active. Wait ~${sec}s, then login once. ` +
              `Do not spam login.`
          });
        }
        await fyersGet(
          ...
          appId,
          accessToken,
          0 // no automatic retries on login probe
        );
      } catch (e) {
        log(`Login validation failed: ${e.message}`, 'error');
        const is429 = /429|rate limit/i.test(e.message);
        return send(res, is429 ? 429 : 401, {
          ok: false,
          error: is429
            ? `FYERS rate limited. Wait 1–2 minutes, then try login once.`
            : `FYERS auth failed: ${e.message}`
        });
      }
