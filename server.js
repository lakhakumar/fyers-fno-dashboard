 ...
 */
const FYERS_SAFE_REQUESTS_PER_MINUTE = 90;
const FYERS_MIN_REQUEST_GAP_MS = 700;
...
function noteFyers429() {
  fyers429Count += 1;
  // Escalating cooldown: 5s, 10s, 20s… capped at 60s
...
async function fyersGet(base, endpoint, params, appId, token, retries = 3) {
...
async function rebuildHistoryFromFyers(appId, token) {
  ...
  // Never wipe or re-fetch the whole day if Render DB already has snapshots
  if (await dbHasDayHistory(date)) {
    if (!state.history.size) await loadDbHistory(date);
    log(`Persisted DB history for ${date} will be used — no FYERS full-day rebuild.`);
    return;
  }
...
async function route(req, res) {
  ...
  try {
    ...
    if (pathName === '/api/login' && req.method === 'POST') {
      ...
      /*
       * Fast path: ranks + live quotes are ready.
       * D-1 / OR / VWAP / volume-spike metrics (~400 rate-limited history
       * calls) run in the background so login is not blocked for 2–5 minutes.
       */
      startFyersSocket();
      broadcastDashboard();

      // Wait until rank rebuild finishes, then load Scanner metrics (avoids 429 at open)
      setTimeout(() => {
        (async () => {
          try {
            if (historyRebuildPromise) {
              log('Background metrics waiting for history rebuild to finish…');
              await historyRebuildPromise;
            }
            await sleep(15000);
            log('Background: loading D-1 levels for Scanner filters…');
          ...
          try {
            await sleep(15000);
            log('Background: loading OR / VWAP / volume-spike metrics…');
            await loadIntradayMetrics(appId, accessToken);
            broadcastDashboard();
            log('Background metrics complete — Scanner filters fully ready.');
          } catch (e) {
            state.metrics.intraLoading = false;
